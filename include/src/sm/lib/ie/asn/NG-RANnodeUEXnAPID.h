/*
 * Generated by asn1c-0.9.29 (http://lionet.info/asn1c)
 * From ASN.1 module "E2SM-COMMON-IEs"
 * 	found in "e2sm_kpm_v03.00_standard.asn1"
 * 	`asn1c -no-gen-BER -no-gen-UPER -no-gen-OER -no-gen-XER -fcompound-names -no-gen-example -findirect-choice -fno-include-deps -S ../../../../../../../asn1c_oai/skeletons/`
 */

#ifndef	_NG_RANnodeUEXnAPID_H_
#define	_NG_RANnodeUEXnAPID_H_


#include <asn_application.h>

/* Including external dependencies */
#include <NativeInteger.h>

#ifdef __cplusplus
extern "C" {
#endif

/* NG-RANnodeUEXnAPID */
typedef unsigned long	 NG_RANnodeUEXnAPID_t;

/* Implementation */
extern asn_per_constraints_t asn_PER_type_NG_RANnodeUEXnAPID_constr_1_mir_asn_common;
extern asn_TYPE_descriptor_t asn_DEF_NG_RANnodeUEXnAPID_mir_asn_common;
extern const asn_INTEGER_specifics_t asn_SPC_NG_RANnodeUEXnAPID_specs_1;
asn_struct_free_f NG_RANnodeUEXnAPID_free;
asn_struct_print_f NG_RANnodeUEXnAPID_print;
asn_constr_check_f NG_RANnodeUEXnAPID_constraint_mir_asn_common;
jer_type_encoder_f NG_RANnodeUEXnAPID_encode_jer;
per_type_decoder_f NG_RANnodeUEXnAPID_decode_aper;
per_type_encoder_f NG_RANnodeUEXnAPID_encode_aper;

#ifdef __cplusplus
}
#endif

#endif	/* _NG_RANnodeUEXnAPID_H_ */
#include <asn_internal.h>
